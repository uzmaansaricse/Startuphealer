      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {services.map((service, idx) => (
            ...
            >
              ...
              <ul className="space-y-2 mb-4 flex-grow">
                {service.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-start text-gray-700 text-xs">
                    <span className="mr-2 mt-0.5 flex-shrink-0 w-4 h-4 rounded-full bg-emerald-100 flex items-center justify-center">
              ...
              {/* Benefits */}
              <div className="mb-4 pt-4 border-t border-emerald-100">
                <p className="text-xs font-semibold text-gray-700 mb-2">Key Benefits:</p>
                <div className="flex flex-wrap gap-2">
                  {service.benefits.map((benefit, bIdx) => (
                    <span
                      key={bIdx}
                      className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full border border-emerald-200"
                    >
                      {benefit}
                    </span>
                  ))}
              ...
              {/* CTA Button */}
              <motion.button
                className="w-full py-3 px-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-semibold rounded-lg 
        ...
        >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: FaShieldAlt, title: "100% Secure", desc: "Your data is safe with us" },
              { icon: FaClock, title: "Fast Delivery", desc: "Quick turnaround times" },
              { icon: FaCheckCircle, title: "Expert Support", desc: "Experienced professionals" },
              { icon: FaRocket, title: "Results Focused", desc: "Measurable outcomes" },
            ].map((item, idx) => (
              <div key={idx} className="text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-emerald-100 to-teal-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <item.icon className="text-emerald-600 text-3xl" />
                </div>
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <motion.a
              href="/contact"
              className="inline-block bg-white text-emerald-600 px-10 py-4 rounded-full font-semibold shadow-lg hover:shadow-xl hover:bg-emerald-50 transition-all duration-300"
            ...
            <motion.a
              href="tel:9610332259"
              className="inline-block bg-transparent border-2 border-white text-white px-10 py-4 rounded-full font-semibold hover:bg-white hover:text-emerald-600 transition-all duration-300"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              Call: +91 9610332259
