      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-extrabold text-gray-800 mb-4">
            Frequently Asked <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 via-emerald-600 to-green-600">Questions</span>
          </h2>
          ...
          >
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 bg-white/95 backdrop-blur-md border-2 border-cyan-200 rounded-2xl 
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto mb-8">
            {[
              ...
              <motion.div
                key={idx}
                className="bg-white/90 backdrop-blur-md rounded-xl p-4 shadow-md border border-cyan-100"
          ...
          >
            {filteredFAQs.length > 0 ? (
              filteredFAQs.map((faq, index) => (
                <motion.div
                  key={faq.id}
                  className="rounded-2xl overflow-hidden bg-white/95 backdrop-blur-md border-2 border-cyan-100 shadow-md hover:shadow-xl transition-all duration-300"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  <button
                    ...
                    className="w-full px-6 py-5 text-left flex items-start justify-between bg-gradient-to-r from-cyan-50 to-emerald-50 hover:from-cyan-100 hover:to-emerald-100 transition-all duration-300 group"
                  >
                    <div className="flex items-start flex-1 pr-4">
                      <FaQuestionCircle className="text-cyan-600 mr-3 mt-1 flex-shrink-0" />
                      <span className="font-semibold text-gray-800 text-base md:text-lg group-hover:text-cyan-700 transition-colors">
                        {faq.question}
                      </span>
                    </div>
                    <span className="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full bg-gradient-to-r from-cyan-500 to-emerald-500 text-white font-bold text-xl group-hover:from-cyan-600 group-hover:to-emerald-600 group-hover:rotate-90 transition-all duration-300">
                      {openIndex === index ? '−' : '+'}
                    </span>
                  </button>
                  ...
                  <AnimatePresence>
                    {openIndex === index && (
                      ...
                      >
                        <div className="px-6 py-5 text-gray-700 bg-white leading-relaxed border-t-2 border-cyan-100">
              ...
              <motion.div
                className="text-center py-12 bg-white/90 backdrop-blur-md rounded-2xl border-2 border-cyan-100"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              >
        ...
        {/* Popular Topics */}
        <motion.div
          className="mb-16 bg-gradient-to-r from-cyan-50 to-emerald-50 rounded-3xl p-8 border-2 border-cyan-200"
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          viewport={{ once: true }}
        >
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              ...
              <div key={idx} className="bg-white/90 rounded-xl p-5 shadow-md border border-cyan-100">
                  ...
                  {topic.queries.map((query, qIdx) => (
                    <li key={qIdx}>
                      <button
                        ...
                        className="text-sm text-cyan-600 hover:text-cyan-700 hover:underline transition-colors text-left"
                      >
                        • {query}
                      </button>
                    </li>
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <motion.a
              href="/contact"
              className="inline-flex items-center justify-center gap-2 bg-white text-cyan-600 px-8 py-4 rounded-full 
                font-bold shadow-lg hover:shadow-xl hover:bg-cyan-50 transition-all duration-300"
            ...
            <motion.a
              ...
              className="inline-flex items-center justify-center gap-2 bg-transparent border-2 border-white text-white px-8 py-4 rounded-full 
                font-bold hover:bg-white hover:text-cyan-600 transition-all duration-300"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <FaPhoneAlt />
